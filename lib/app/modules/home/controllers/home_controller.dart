...
import 'package:financeapp/app/modules/Tab/controllers/tab_controller.dart';
...
// Method to apply title filter 1
  ...
  //     filteredItemsm.assignAll(lastFilteredItemsm);// Replace with actual full list variable
  //   } else {
  //     // Split the title filter by space to handle multiple titles
  //     List<String> titles = titleFilter.value.toLowerCase().split(',');
  //
  //     // Apply title filter
  //     var filtered = filteredItemsm.where((item) {
  ...
  //     if (filtered.isEmpty) {
  //       print("No matches found");
  //       // Optionally, you can clear the filtered list or handle it in a way that the UI remains stable
  //       filteredItemsm.clear();
  //     } else {
  //       filteredItemsm.assignAll(filtered);
  //     }
  //   }
  ...
  //   if (titleFilter.value.isEmpty) {
  //     // No title filter applied, use the full list
  //     // Assuming the full list is stored in another variable or fetched again
  //     filteredItemsm.value = filteredItemsm; // Replace with actual full list
  //   } else {
  //     // Split the title filter by space to handle multiple titles
  //     List<String> titles = titleFilter.value.toLowerCase().split('');
  //
  //     // Apply title filter
  //     filteredItemsm.value = filteredItemsm.where((item) {
  //       String itemTitleLower = item.title.toLowerCase();
  //       for (var title in titles) {
  //         if (!itemTitleLower.contains(title)) {
  //           return false; // If any title does not match, exclude this item
  ...
  //       }
  //       return true;
  //       // return titles.every((title) => itemTitleLower.contains(title));// Include item if all titles match
  //     }).toList();
  //   }
  //   // Apply date filter if already applied
  //   // filterItemsByMonthYear(pickedDate.value);
  // }
  //
  // void applyTitleFilter1() {
  //   filterItemsByMonthYear1(pickedDate1.value);
  //   if (titleFilter1.value.isEmpty) {
  //     // No title filter applied, use the full list
  //     // Assuming the full list is stored in another variable or fetched again
  //     filteredItemsm1.value =  filteredItemsm1; // Replace with actual full list
  //   } else {
  //     // Split the title filter by space to handle multiple titles
  //     List<String> titles = titleFilter1.value.toLowerCase().split(',');
  //
  //     // Apply title filter
  //     filteredItemsm1.value = filteredItemsm1.where((item) {
  //       String itemTitleLower = item.title.toLowerCase();
  //       for (var title in titles) {
  //         if (!itemTitleLower.contains(title)) {
  //           return false; // If any title does not match, exclude this item
...
//filter transaction page
  ...
  Future<void> onInit() async {
    ...
    items();
    items1();
    // calculateAndSetDifference("Transport");
    // calculateAndSetDifference("Education");
    // calculateAndSetDifference("Food");
    differenceAmount;
    ina;
    updateTotalAmount();
    updateTotalAmount1();
    ...
    // addItem(key: '', title: '', subTitle: '', amount: '', time: '', gst: '', receipt: '', invoice: '', vendor: '', tag: '', account: '');
    // addItem1(key: '', title: '', subTitle: '', amount: '', time: '', tag: '');
    // writeTitleToStorage('', '', '', '', '', '', '', '', '', '', '' as File?);
...
///today,yesterday main
  ...
  //       total += amount;
  //     } catch (e) {
  //       print('Error parsing amount for item ${item.key}: ${item.amount} - $e');
  //       // Optionally handle the error or skip this item
  //     }
  //   }
...
//linearchartend
// Map<String, double> aggregateData(List<CustomListItem> items, String type) {
//   Map<String, double> data = {};
//
//   for (var item in items) {
//     DateTime date = DateTime.parse(item.time);
